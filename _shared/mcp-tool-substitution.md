...
- *Hangs with no signal at all*: Continue CLI (confirmed live, worst case — no log line, no exit, nothing), Google Antigravity (multiple GitHub issues; also see Gemini CLI's now-historical entry below — it reported the same failure mode before Google deprecated it in favor of Antigravity).
...
exposes it) are both "coexistence only" in practice. Warp, Junie, and QM have no such mechanism at any
layer — MCP is architecturally additive-only for these three. **GitHub Copilot is a distinct third
case**: the mechanism exists and is exposed (`--available-tools`/`--excluded-tools`/

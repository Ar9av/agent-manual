...
> ⚠️ **Live-tested 2026-07-23 — hooks did NOT fire in headless (`-p`) mode.** On Ubuntu 24.04 with `cn` v1.5.47, configured a real `PreToolUse` hook in `.continue/settings.json` (the documented location, confirmed correct by reading `extensions/cli/src/hooks/hookConfig.ts` directly — precedence order is `~/.claude/settings.json` < `.claude/settings.json` < `~/.continue/settings.json` < `.continue/settings.json` < `.continue/settings.local.json`). Ran `cn --config ./continue-config.yaml --auto -p "Run the exact shell command: echo forbidden-marker-xyz"` with a hook script that should have blocked any command containing that marker string. **The hook never fired** — the debug log showed the `Bash` tool executing normally straight after "Checking tool permissions," with zero log lines mentioning hooks anywhere in the session, and the hook's own debug-log side effect (which should always write, block-or-allow) never appeared. This matches a real code-level caveat in `extensions/cli/src/hooks/fireHook.ts`: hook-firing calls are explicitly designed to "be safe to call even if the HookService is not yet initialized (they return no-op results)" — i.e. **hooks fail silently open with no warning if the HookService isn't ready when a tool runs**, which is exactly the symptom observed. Root cause (service-init ordering under `-p`/`--config`, specifically) wasn't pinned down further, but the practical takeaway is: **do not assume a hook configured per this section is actually active — verify it fires (e.g. have it always write to a log file) before relying on it for anything security-sensitive**, especially in headless/CI use.

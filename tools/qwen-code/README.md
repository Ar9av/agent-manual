...
> **Live-tested 2026-07-23 on qwen-code v0.20.1 — fully confirmed on retest with a stronger model.** An initial pass with `gpt-4o-mini` in default (non-yolo) mode gave confusing, seemingly-fabricated results: the model narrated plausible "command succeeded" and "blocked by your hook" outcomes for the shell tool without a real `run_shell_command` tool call ever appearing in the session transcript or hook log. Root cause: **non-interactive mode (`-p`) requires explicit `-y`/`--yolo` for shell tool calls to auto-execute at all** — without it, tool approval silently cannot proceed in a non-interactive session, and the weaker model filled the gap by narrating a plausible-sounding result instead of surfacing the failure. Re-running with `qwen -y -p "..."` and `gpt-4o` (env: `OPENAI_MODEL`, `OPENAI_API_KEY`, `OPENAI_BASE_URL=https://api.openai.com/v1`) gave a clean, fully-confirmed result: a `PreToolUse` hook matching `run_shell_command` fired correctly with the exact documented stdin schema (`hook_event_name`, `tool_name: "run_shell_command"`, `tool_input.command`, etc.), and a hook returning `{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"..."}}` genuinely blocked the shell command (the model reported it was "unable to run" the command). **Practical takeaway:** if you're scripting/testing Qwen Code hooks non-interactively, always pass `-y`, and don't trust a weak/cheap model's narration of tool outcomes as proof anything actually happened — check the hook's own logs or the session transcript.
...
> **Live-verified 2026-07-23: real, working delegation confirmed via a built-in `agent` tool.** Prompted `qwen -y -p "Spawn a subagent to create subagent-test.txt containing SUBAGENT_WORKED, then confirm."` (v0.20.1, `gpt-4o`) and the session transcript showed a genuine `agent` tool call with its own `description`/`prompt` fields and a distinct `response.output` — the file was created correctly. This is a real, working subagent mechanism (comparable to Claude Code's `Task` tool, Goose's `delegate`, etc.), not just advertised. Exact configuration for defining custom subagent "teams"/roles beyond the default single-shot `agent` tool call remains unconfirmed by this test.
